...
from Rally.helper.Rally_config import sql_fetch_syscon_defect, sql_trunc_defect, sql_trunc_aa_defect, \
    sql_active_artifacts_defect, sql_insert_defect, fields_defect, fields_clob_defect, db_pkg_name_defect, \
    log_file_path_defect, process_id_defect
...
log_file = log_file_path_defect + str(dt_log) + '.log'
...
l_run = c_log.callproc('BMR_LOG.INSERT_PROCESS_RUN_DTL', (today, process_id_defect, env, run_stat, 0))
l_run_id = l_run[4]
...
try:
    sys_con, query = prepare_api_query(entity_type, sql_fetch_syscon_defect, cur_sys_config)
...
try:
    ...
    c_trunc.execute(sql_trunc_defect)
    c_trunc.execute(sql_trunc_aa_defect)
...
for project_name in PROJECTS:
    ...
    try:
        for batch in range(1, batches + 1):
            ...
            #fetch_fields = ','.join(fields_defect)
            ...
            flag_status = paginate_all_rows_batch(l_run_id, c_log, batch, sql_insert_defect, df, entity_type,
                                                  fields_defect, flag_status, hdlr, fields_clob_defect)
...
if flag_status == 'Y':
    ...
    try:
        ...
        c_proc_call.callproc(db_pkg_name_defect,[l_run_id])
        c_proc_call.close()
