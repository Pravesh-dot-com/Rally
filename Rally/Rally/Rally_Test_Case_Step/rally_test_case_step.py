...
from Rally.helper.Rally_config import sql_fetch_syscon_test_case_step, sql_trunc_test_case_step, \
    sql_insert_test_case_step, fields_test_case_step, fields_clob_test_case_step, db_pkg_name_test_case_step, \
    log_file_path_test_case_step, process_id_test_case_step
...
entity_type = 'TestCaseStep'

# Set file logging #
today = dt.datetime.now()
dt_log = today.strftime('%d_%b_%Y_%I_%M_%p')
log_file = log_file_path_test_case_step + str(dt_log) + '.log'
...
l_run = c_log.callproc('BMR_LOG.INSERT_PROCESS_RUN_DTL', (today, process_id_test_case_step, env, run_stat, 0))
l_run_id = l_run[4]
...
try:
    sys_con, query = prepare_api_query(entity_type, sql_fetch_syscon_test_case_step, cur_sys_config)
...
c_trunc = conn.cursor()
try:
    c_trunc.execute(sql_trunc_test_case_step)
    c_trunc.close()
...
for project_name in PROJECTS:
    ...
    try:
        for batch in range(1, batches + 1):
            ...
            flag_status = paginate_all_rows_batch(l_run_id, c_log, batch, sql_insert_test_case_step, df, entity_type,
                                                  fields_test_case_step, flag_status, hdlr, fields_clob_test_case_step)
...
if flag_status == 'Y':
    ...
    try:
        ...
        c_proc_call.callproc(db_pkg_name_test_case_step,[l_run_id])
        c_proc_call.close()
