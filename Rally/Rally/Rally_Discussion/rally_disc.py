...
from Rally.helper.Rally_config import sql_fetch_syscon_disc, sql_trunc_disc, sql_insert_disc, fields_disc, \
    fields_clob_disc, db_pkg_name_disc, log_file_path_disc, process_id_disc
...
entity_type = 'ConversationPost'

# Set file logging #
today = dt.datetime.now()
dt_log = today.strftime('%d_%b_%Y_%I_%M_%p')
log_file = log_file_path_disc + str(dt_log) + '.log'
...
c_trunc = conn.cursor()
try:
    c_trunc.execute(sql_trunc_disc)
    c_trunc.close()
...
if flag_status == 'Y':
    ...
    try:
        ...
        c_proc_call.callproc(db_pkg_name_disc,[l_run_id])
        c_proc_call.close()
