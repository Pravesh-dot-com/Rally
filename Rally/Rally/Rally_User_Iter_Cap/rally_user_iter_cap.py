...
from Rally.helper.Rally_config import sql_fetch_syscon_user_iter_cap, sql_trunc_user_iter_cap, sql_insert_user_iter_cap, fields_user_iter_cap, \
    fields_clob_user_iter_cap, db_pkg_name_user_iter_cap, log_file_path_user_iter_cap, process_id_user_iter_cap
...
entity_type = 'UserIterationCapacity'

# Set file logging #
today = dt.datetime.now()
dt_log = today.strftime('%d_%b_%Y_%I_%M_%p')
log_file = log_file_path_user_iter_cap + str(dt_log) + '.log'
...
c_trunc = conn.cursor()
try:
    c_trunc.execute(sql_trunc_user_iter_cap)
    c_trunc.close()
...
if flag_status == 'Y':
    ...
    try:
        ...
        c_proc_call.callproc(db_pkg_name_user_iter_cap,[l_run_id])
        c_proc_call.close()
