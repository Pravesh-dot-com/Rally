...
from Rally.helper.Rally_config import sql_fetch_syscon_test_case_result, sql_trunc_test_case_result, sql_insert_test_case_result, fields_test_case_result, \
    fields_clob_test_case_result, db_pkg_name_test_case_result, log_file_path_test_case_result, process_id_test_case_result
...
log_file = log_file_path_test_case_result + str(dt_log) + '.log'
...
l_run = c_log.callproc('BMR_LOG.INSERT_PROCESS_RUN_DTL', (today, process_id_test_case_result, env, run_stat, 0))
l_run_id = l_run[4]
...
try:
    sys_con, query = prepare_api_query(entity_type, sql_fetch_syscon_test_case_result, cur_sys_config)
...
c_trunc = conn.cursor()
try:
    c_trunc.execute(sql_trunc_test_case_result)
    c_trunc.close()
    stat = "Staging Data Table Truncated"
    logging.info(stat)
    c_log.callproc('BMR_LOG.INSERT_PROCESS_RUN_LOG_DTL', (l_run_id, info, stag, stat))
except:
...
for project_name in PROJECTS:
    ...
    try:
        for batch in range(1, batches + 1):
            ...
            #fetch_fields = ','.join(fields_test_case_result)
            ...
            flag_status = paginate_all_rows_batch(l_run_id, c_log, batch, sql_insert_test_case_result, df, entity_type,
                                                  fields_test_case_result, flag_status, hdlr, fields_clob_test_case_result)
...
if flag_status == 'Y':
    ...
    try:
        ...
        c_proc_call.callproc(db_pkg_name_test_case_result,[l_run_id])
        c_proc_call.close()
