...
from Rally.helper.Rally_config import db_con_str, run_stat, env, stag, target, info, warn, err, flag_status, \
    sql_delete_tag_assignment, tag_entity, PROJECTS
# import dynamic variables #
from Rally.helper.Rally_config import sql_fetch_syscon_defect_suite, sql_trunc_defect_suite, sql_trunc_aa_defect_suite, \
    sql_active_artifacts_defect_suite, sql_insert_defect_suite, fields_defect_suite, fields_clob_defect_suite, db_pkg_name_defect_suite, \
    log_file_path_defect_suite, process_id_defect_suite
...
entity_type = 'DefectSuite'

# Set file logging #
today = dt.datetime.now()
dt_log = today.strftime('%d_%b_%Y_%I_%M_%p')
log_file = log_file_path_defect_suite + str(dt_log) + '.log'
...
l_run = c_log.callproc('BMR_LOG.INSERT_PROCESS_RUN_DTL', (today, process_id_defect_suite, env, run_stat, 0))
l_run_id = l_run[4]
...
try:
    sys_con, query = prepare_api_query(entity_type, sql_fetch_syscon_defect_suite, cur_sys_config)
...
try:
    ...
    c_trunc.execute(sql_trunc_defect_suite)
    c_trunc.execute(sql_trunc_aa_defect_suite)
...
c1 = conn.cursor()
try:
    get_active_artifacts(l_run_id, c_log, c1, entity_type, sql_active_artifacts_defect_suite, hdlr)
    c1.close()
...
for project_name in PROJECTS:
    ...
    try:
        for batch in range(1, batches + 1):
            ...
            #fetch_fields = ','.join(fields_defect_suite)
            ...
            flag_status = paginate_all_rows_batch(l_run_id, c_log, batch, sql_insert_defect_suite, df, entity_type,
                                                  fields_defect_suite, flag_status, hdlr, fields_clob_defect_suite)
...
if flag_status == 'Y':
    ...
    try:
        ...
        c_proc_call.callproc(db_pkg_name_defect_suite,[l_run_id])
        c_proc_call.close()
