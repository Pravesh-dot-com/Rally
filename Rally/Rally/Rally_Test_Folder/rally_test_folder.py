...
from Rally.helper.Rally_config import sql_fetch_syscon_test_folder, sql_trunc_test_folder, sql_insert_test_folder, fields_test_folder, \
    fields_clob_test_folder, db_pkg_name_test_folder, log_file_path_test_folder, process_id_test_folder
...
log_file = log_file_path_test_folder + str(dt_log) + '.log'
...
l_run = c_log.callproc('BMR_LOG.INSERT_PROCESS_RUN_DTL', (today, process_id_test_folder, env, run_stat, 0))
l_run_id = l_run[4]
...
try:
    sys_con, query = prepare_api_query(entity_type, sql_fetch_syscon_test_folder, cur_sys_config)
...
c_trunc = conn.cursor()
try:
    c_trunc.execute(sql_trunc_test_folder)
    c_trunc.close()
...
for project_name in PROJECTS:
    ...
    try:
        for batch in range(1, batches + 1):
            ...
            flag_status = paginate_all_rows_batch(l_run_id, c_log, batch, sql_insert_test_folder, df, entity_type,
                                                  fields_test_folder, flag_status, hdlr, fields_clob_test_folder)
...
if flag_status == 'Y':
    ...
    try:
        ...
        c_proc_call.callproc(db_pkg_name_test_folder,[l_run_id])
        c_proc_call.close()
