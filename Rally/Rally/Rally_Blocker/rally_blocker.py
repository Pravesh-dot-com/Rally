...
from Rally.helper.Rally_config import sql_fetch_syscon_blocker, sql_trunc_blocker, sql_insert_blocker, fields_blocker, \
    fields_clob_blocker, db_pkg_name_blocker, log_file_path_blocker, process_id_blocker
...
log_file = log_file_path_blocker + str(dt_log) + '.log'
...
l_run = c_log.callproc('BMR_LOG.INSERT_PROCESS_RUN_DTL', (today, process_id_blocker, env, run_stat, 0))
l_run_id = l_run[4]
...
try:
    sys_con, query = prepare_api_query(entity_type, sql_fetch_syscon_blocker, cur_sys_config)
...
c_trunc = conn.cursor()
try:
    c_trunc.execute(sql_trunc_blocker)
    c_trunc.close()
...
for project_name in PROJECTS:
    ...
    if not query_errors:  # and total > 0:
        try:
            ...
            if total == 0:
                ...
                stat = 'API response iterator fetch complete for Project {} and entity - {} ' \
                       'and fetch_query - {}'.format(project_name,entity_type,query)
                logging.info(stat)
                c_log.callproc('BMR_LOG.INSERT_PROCESS_RUN_LOG_DTL', (l_run_id, info, stag, stat))
    ...
    try:
        for batch in range(1, batches + 1):
            ...
            flag_status = paginate_all_rows_batch(l_run_id, c_log, batch, sql_insert_blocker, df, entity_type,
                                                  fields_blocker, flag_status, hdlr, fields_clob_blocker)
...
if flag_status == 'Y':
    ...
    try:
        ...
        c_proc_call.callproc(db_pkg_name_blocker,[l_run_id])
        c_proc_call.close()
