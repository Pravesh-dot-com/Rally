...
try:
    sys_con, query = prepare_api_query(entity_type, sql_fetch_syscon_pi, cur_sys_config)
...
for project_name in PROJECTS:
    ...
    try:
        for batch in range(1, batches + 1):
            ...
            flag_status = paginate_all_rows_batch(l_run_id, c_log, batch, sql_insert_pi, df, entity_type,
                                                  fields_pi, flag_status, hdlr, fields_clob_pi)
...
if flag_status == 'Y':
    ...
    try:
        ...
        c_proc_call.callproc(db_pkg_name_pi,[l_run_id])
        c_proc_call.close()
