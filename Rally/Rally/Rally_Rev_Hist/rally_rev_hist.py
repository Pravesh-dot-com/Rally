...
from Rally.helper.Rally_config import sql_fetch_syscon_rev_hist, sql_trunc_rev_hist, sql_insert_rev_hist, \
    fields_rev_hist, \
    fields_clob_rev_hist, db_pkg_name_rev_hist, log_file_path_rev_hist, process_id_rev_hist
...
log_file = log_file_path_rev_hist + str(dt_log) + '.log'
...
l_run = c_log.callproc('BMR_LOG.INSERT_PROCESS_RUN_DTL', (today, process_id_rev_hist, env, run_stat, 0))
l_run_id = l_run[4]
...
try:
    sys_con, query = prepare_api_query(entity_type, sql_fetch_syscon_rev_hist, cur_sys_config)
...
c_trunc = conn.cursor()
try:
    c_trunc.execute(sql_trunc_rev_hist)
    c_trunc.close()
...
for project_name in PROJECTS:
    ...
    try:
        for batch in range(1, batches + 1):
            ...
            # fetch_fields = ','.join(fields_rev_hist)
            ...
            flag_status = paginate_all_rows_batch(l_run_id, c_log, batch, sql_insert_rev_hist, df, entity_type,
                                                  fields_rev_hist, flag_status, hdlr, fields_clob_rev_hist)
...
if flag_status == 'Y':
    ...
    try:
        ...
        c_proc_call.callproc(db_pkg_name_rev_hist,[l_run_id])
        c_proc_call.close()
