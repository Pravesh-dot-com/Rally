...
from Rally.helper.Rally_config import sql_fetch_syscon_test_set, sql_trunc_test_set, sql_trunc_aa_test_set, \
    sql_active_artifacts_test_set, sql_insert_test_set, fields_test_set, fields_clob_test_set, db_pkg_name_test_set, \
    log_file_path_test_set, process_id_test_set
...
log_file = log_file_path_test_set + str(dt_log) + '.log'
...
l_run = c_log.callproc('BMR_LOG.INSERT_PROCESS_RUN_DTL', (today, process_id_test_set, env, run_stat, 0))
l_run_id = l_run[4]
...
try:
    sys_con, query = prepare_api_query(entity_type, sql_fetch_syscon_test_set, cur_sys_config)
...
try:
    ...
    c_trunc.execute(sql_trunc_test_set)
    c_trunc.execute(sql_trunc_aa_test_set)
...
for project_name in PROJECTS:
    ...
    try:
        for batch in range(1, batches + 1):
            ...
            flag_status = paginate_all_rows_batch(l_run_id, c_log, batch, sql_insert_test_set, df, entity_type,
                                                  fields_test_set, flag_status, hdlr, fields_clob_test_set)
...
# To check if there are any fetch results for all the projects included. Exit when nothing found.
# This scenario can occur for a few entities when run daily since new records won't be added every day.
...
# To check whether the staging process completed successfully or not.
if flag_status == 'Y':
    ...
    try:
        ...
        c_proc_call.callproc(db_pkg_name_test_set,[l_run_id])
        c_proc_call.close()
