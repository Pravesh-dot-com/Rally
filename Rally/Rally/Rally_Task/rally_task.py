...
from Rally.helper.Rally_config import sql_fetch_syscon_task, sql_trunc_task, sql_trunc_aa_task, \
    sql_active_artifacts_task, sql_insert_task, fields_task, fields_clob_task, db_pkg_name_task, log_file_path_task, process_id_task
...
log_file = log_file_path_task + str(dt_log) + '.log'
...
l_run = c_log.callproc('BMR_LOG.INSERT_PROCESS_RUN_DTL', (today, process_id_task, env, run_stat, 0))
l_run_id = l_run[4]
...
try:
    sys_con, query = prepare_api_query(entity_type, sql_fetch_syscon_task, cur_sys_config)
...
try:
    ...
    c_trunc.execute(sql_trunc_task)
    c_trunc.execute(sql_trunc_aa_task)
...
c1 = conn.cursor()
try:
    get_active_artifacts(l_run_id, c_log, c1, entity_type, sql_active_artifacts_task, hdlr)
    c1.close()
...
for project_name in PROJECTS:
    ...
    try:
        for batch in range(1, batches + 1):
            ...
            flag_status = paginate_all_rows_batch(l_run_id, c_log, batch, sql_insert_task, df, entity_type,
                                                  fields_task, flag_status, hdlr, fields_clob_task)
...
if flag_status == 'Y':
    ...
    try:
        ...
        c_proc_call.callproc(db_pkg_name_task,[l_run_id])
        c_proc_call.close()
