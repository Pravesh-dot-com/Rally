...
from Rally.helper.Rally_config import sql_fetch_syscon_test_case, sql_trunc_test_case, sql_trunc_aa_test_case, \
    sql_active_artifacts_test_case, sql_insert_test_case, fields_test_case, fields_clob_test_case, db_pkg_name_test_case, \
    log_file_path_test_case, process_id_test_case
...
log_file = log_file_path_test_case + str(dt_log) + '.log'
...
l_run = c_log.callproc('BMR_LOG.INSERT_PROCESS_RUN_DTL', (today, process_id_test_case, env, run_stat, 0))
l_run_id = l_run[4]
...
try:
    sys_con, query = prepare_api_query(entity_type, sql_fetch_syscon_test_case, cur_sys_config)
...
try:
    ...
    c_trunc.execute(sql_trunc_test_case)
    c_trunc.execute(sql_trunc_aa_test_case)
...
c1 = conn.cursor()
try:
    get_active_artifacts(l_run_id, c_log, c1, entity_type, sql_active_artifacts_test_case, hdlr)
    c1.close()
...
for project_name in PROJECTS:
    ...
    try:
        for batch in range(1, batches + 1):
            ...
            # fetch_fields = ','.join(fields_test_case)
            ...
            flag_status = paginate_all_rows_batch(l_run_id, c_log, batch, sql_insert_test_case, df, entity_type,
                                                  fields_test_case, flag_status, hdlr, fields_clob_test_case)
...
if flag_status == 'Y':
    ...
    try:
        ...
        c_proc_call.callproc(db_pkg_name_test_case,[l_run_id])
        c_proc_call.close()
